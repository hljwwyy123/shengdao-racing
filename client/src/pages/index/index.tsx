...
import { Collapse, Image, Button, PullToRefresh, Picker } from "@nutui/nutui-react-taro"
import { ArrowRight, ArrowDown, ArrowUp } from '@nutui/icons-react-taro'
...
export default function Index() {
  const [originalRankList, setOriginalRankList] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (!originalRankList.length) return;

    const filteredData = selectedGroup === '全部' 
      ? originalRankList 
      : originalRankList.filter(item => item.group === selectedGroup);
    
    setRankList(filteredData);
    const _rankList = cloneDeep(filteredData);
    const noList = _rankList.sort((a, b) => a.single_score - b.single_score);
    setRankNoList(noList);

  }, [selectedGroup, originalRankList]);
  ...
  const getRealTimeScore = async () => {
    try {
      ...
      setOriginalRankList(listData);
      const groups = [...new Set(listData.map(item => item.group).filter(Boolean))];
    ...
    <View>
      <div className='index'>
        ...
        <div className="filter-bar">
          <div className="group-filter" onClick={() => setShowGroupPicker(true)}>
            <span>组别: {selectedGroup}</span>
            <ArrowUp style={{ transform: showGroupPicker ? 'none' : 'rotate(180deg)', transition: 'transform 0.3s' }} />
          </div>
        </div>
        {
          activeTab == 0 &&
                ...
                rankList.map((record: REAL_SCORE_ITEM, index: number) => <Collapse.Item 
                  ...
                  extra={
                    <div className='extra-cell'>
                      ...
                      <div className='group-cell'>{record.group || '-'}</div>
      ...
      <Picker
        ...
        onConfirm={(list, value) => {
          setSelectedGroup(value[0] as string);
